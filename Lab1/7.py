...
def objedini(l1):
    return reduce(lambda a, b: dodajnaprvo(a, b), list(map(lambda x: {x: None} if type(x) in (int, float) else {x[0]: list(
        map(lambda z: z[1], filter(lambda y: y[0] > 0, enumerate(list(x)))))}, l1)))
